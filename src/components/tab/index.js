...
export class Tabs extends ORUIElement {
            ...
            ORUI.createElement("div", { class: 'orui-tabs' },
                ORUI.createElement("div", { class: 'orui-tabs-header' },
                    ORUI.createElement("div", { class: 'orui-tabs-list' },
                        ORUI.createElement("div", { class: 'orui-tabs-line', style: `width:${this.underline_width / 16}rem` }),
                        data.map((_item) => {
    ...
    set_content(active_key) {
        if (this.wraps.length > 0) {
            ...
            if (active_item != this.wraps[item_index]) {
                ...
                if (!this.is_init) {
                    const render = () => {
                        const index = this.get_key_index(this.active_key);
                        const flag = this.set_underline_style(this.wraps[index]);
                        if (flag) {
                            return;
                        }
    ...
    set_underline_style(active_item) {
        let flag = true;
        const width = this.underline_width ? this.underline_width / 16 + 'rem' : (active_item.offsetWidth - 24) / 16 + 'rem';
        if (parseFloat(width) != this._active_underline.clientWidth / 16) {
            flag = false;
        }
        this._active_underline.style.width = width;
        const _width = this.underline_width ? this._active_underline.offsetWidth + 24 : active_item.offsetWidth;
        const _left = (active_item.offsetLeft + 12 + ((active_item.offsetWidth - _width) / 2)) / 16 + 'rem';
        if (parseFloat(_left) != this._active_underline.offsetLeft / 16) {
            flag = false;
        }
